import Header from "../Header";
import Footer from "../Footer";

import banner1 from '../../assets/images/Layer35-cattle.png';
import banner2 from '../../assets/images/Layer37-fish.png';
import banner3 from '../../assets/images/Layer38-poultry.png';
import banner4 from '../../assets/images/Layer39-pig.png';
import contactBaner from '../../assets/images/contact-banner.jpg';
import animal1 from "../../assets/images/cattle1.png";
import animal2 from "../../assets/images/pig2.png";
import animal3 from "../../assets/images/poultry2.png";
import animal4 from "../../assets/images/fish2.png";
import feedbanner from "../../assets/images/cattle-banner-2.jpg"
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import {
  faCow,
  faChartLine,
  faDumbbell,
  faSeedling,
  faShieldHalved,
  faLeaf,
  faEgg,
  faShieldVirus,
  faArrowTrendUp,
  faGaugeHigh,
  faBolt,
  faDrumstickBite,
  faPiggyBank,
  faFish,
  faWater,
  faFlask,
  faRecycle,
  faCheck,
  faArrowRight,
} from "@fortawesome/free-solid-svg-icons";
import { faWhatsapp } from "@fortawesome/free-brands-svg-icons";
import { Link } from "react-router-dom";
import nutritionHero from "../../assets/images/nutrition-banner.png";

function FeedType(){
  return (
    <>
      <Header></Header>
      <main className="pt-16 overflow-x-hidden">
        {/* Hero Section */}
        <section className="relative z-0">
          <div className="relative">
            <img
              src={feedbanner}
              alt="Contact Us Banner"
              className="w-full md:h-[500px] h-[350px] object-cover"
            />
            <div className="absolute inset-0 bg-gradient-to-r from-gray-900/80 via-gray-900/60 to-transparent"></div>
            <div className="absolute inset-0  flex items-center justify-center flex-col ">
              <h1 className="text-white text-4xl md:text-6xl font-bold max-w-6xl px-4">
                Types of feeds
              </h1>

              <p className="text-gray-200 text-[16px] md:text-xl text-center max-w-6xl mt-6">
                At Green Gold Animal Feed, our nutrition programs are designed
                using scientific nutrient requirements, feed conversion ratios
                (FCR), and performance calculations to deliver measurable
                results for farmers.
              </p>
            </div>
          </div>
        </section>

        {/* ================= CATTLE ================= */}
        <section className="py-2  md:py-10">
          <div className="text-center  mb-6">
            <h2 className="text-3xl md:text-5xl font-semibold text-gray-800 text-center ">
              Cattle <span className="text-[#ffa800]">Feed</span>
            </h2>
          </div>
          <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-2 items-center gap-4 md:gap-12">
            <img
              src={animal1}
              className="block w-full h-[280px]  md:h-[350px] object-cover"
              alt=""
            />

            <div className="space-y-3">
              <div
                className="relative w-full h-auto bg-white rounded-2xl p-8 space-y-5
                     shadow-[0_15px_40px_rgba(0,0,0,0.15)] z-30"
              >
                <h3 className="text-[22px] md:text-[24px] font-bold text-gray-800">
                  Cattle Concentrate Feed
                </h3>

                <ul className="space-y-3 text-sm text-gray-700">
                  <li className="flex items-center gap-3">
                    <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                      <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                    </span>
                    Protein: 16–18%
                  </li>
                  <li className="flex items-center gap-3">
                    <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                      <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                    </span>
                    Enhances digestion & feed efficiency
                  </li>
                  <li className="flex items-center gap-3">
                    <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                      <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                    </span>
                    Suitable for dairy cattle & buffaloes
                  </li>
                </ul>
              </div>
              <div
                className="relative w-full h-auto bg-white rounded-2xl p-8 space-y-5
                     shadow-[0_15px_40px_rgba(0,0,0,0.15)] z-30"
              >
                <h3 className="text-[22px] md:text-[24px] font-bold text-gray-800">
                  Calf Starter Feed
                </h3>

                <ul className="space-y-3 text-sm text-gray-700">
                  <li className="flex items-center gap-3">
                    <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                      <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                    </span>
                    Protein: 20–22%
                  </li>
                  <li className="flex items-center gap-3">
                    <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                      <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                    </span>
                    Supports early rumen development
                  </li>
                  <li className="flex items-center gap-3">
                    <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                      <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                    </span>
                    Promotes faster & healthier growth
                  </li>
                  <li className="flex items-center gap-3">
                    <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                      <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                    </span>
                    Suitable for calves (up to 6 months)
                  </li>
                </ul>
              </div>
            </div>
          </div>
          {/* Feeding Schedule */}
          <div className=" mt-14 max-w-7xl mx-auto px-4 ">
            <h3 className="text-2xl font-semibold text-gray-800 flex items-center gap-3 mb-6 text-center justify-center">
              {/* <FontAwesomeIcon icon={faClipboardList} /> */}
              Cattle Feeding Schedule
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8  bg-white rounded-2xl p-4 md:p-8 shadow-sm">
              <div>
                {/* Calves */}
                <div className="mb-8">
                  <h4 className="text-lg font-semibold text-gray-800 mb-3 md:text-left  text-center">
                    {/* <FontAwesomeIcon icon={faCow} /> */}
                    Calves (0–6 Months)
                  </h4>

                  <div className="overflow-x-auto  border border-green-300 rounded-xl">
                    <table className="w-full border border-green-600 rounded-xl border-collapse">
                      <thead className="bg-green-100 text-gray-800">
                        <tr>
                          <th className="px-4 py-3 text-center text-sm md:text-base font-semibold border border-green-200">
                            Age
                          </th>
                          <th className="px-4 py-3 text-center text-sm md:text-base font-semibold border border-green-200">
                            Feed Type
                          </th>
                          <th className="px-4 py-3 text-center text-sm md:text-base font-semibold border border-green-200">
                            Quantity / Day
                          </th>
                        </tr>
                      </thead>

                      <tbody className="text-gray-700">
                        <tr className="hover:bg-green-50 transition">
                          <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                            0–3 months
                          </td>
                          <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                            Calf Starter
                          </td>
                          <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                            0.5 – 1.0 kg
                          </td>
                        </tr>

                        <tr className="hover:bg-green-50 transition">
                          <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                            3–6 months
                          </td>
                          <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                            Calf Starter + Green Fodder
                          </td>
                          <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                            1.0–2.0 kg
                          </td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                </div>
              </div>

              {/* Growing Cattle */}
              <div>
                <h4 className="text-lg font-semibold text-gray-800 mb-3 md:text-left  text-center">
                  {/* <FontAwesomeIcon icon={faCow} /> */}
                  Growing Cattle (6–24 Months)
                </h4>
                <div className="overflow-x-auto  border border-green-300 rounded-xl">
                  <table className="w-full border border-green-600 rounded-xl border-collapse">
                    <thead className="bg-green-100 text-gray-800">
                      <tr>
                        <th className="px-4 py-3 text-center text-sm md:text-base font-semibold border border-green-200">
                          Feed Type
                        </th>

                        <th className="px-4 py-3 text-center text-sm md:text-base font-semibold border border-green-200">
                          Quantity / Day
                        </th>
                      </tr>
                    </thead>

                    <tbody className="text-gray-700">
                      <tr className="hover:bg-green-50 transition">
                        <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          Cattle Feed
                        </td>
                        <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          2 – 3 kg
                        </td>
                      </tr>

                      <tr className="hover:bg-green-50 transition">
                        <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          Green Fodder
                        </td>
                        <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          10–15 kg
                        </td>
                      </tr>

                      <tr className="hover:bg-green-50 transition">
                        <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          Dry Fodder
                        </td>
                        <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          4–5 kg
                        </td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              </div>
            </div>
          </div>
        </section>

        {/* ================= Poultry ================= */}
        <section className="py-4  md:py-10">
          <div className="text-center  mb-6">
            {/* <h2 className="text-3xl md:text-5xl font-semibold text-green-800 flex items-center justify-center gap-3">
                Feed
              </h2> */}
            <h2 className="text-3xl md:text-5xl font-semibold text-gray-800 text-center">
              Poultry <span className="text-yellow-600">Feed</span>
            </h2>
            <p className="text-gray-600 mt-3 max-w-2xl mx-auto text-[16px] md:text-[18px]">
              Proper feeding at the right age and quantity ensures better
              growth, lower feed cost, and higher productivity.
            </p>
          </div>

          <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 items-stretch gap-4 md:gap-6">
            {/* card-1 */}
            <div className="relative w-full h-full bg-white rounded-2xl p-8 shadow-[0_15px_40px_rgba(0,0,0,0.15)] z-30 flex flex-col">
              <span className="w-[40px] h-[40px] bg-[#00a63e] rounded-full block text-white text-center mx-auto leading-[40px] mb-4">
                <FontAwesomeIcon icon={faDrumstickBite} />
              </span>

              <h2 className="text-lg font-bold text-gray-900 mb-2 text-center min-h-[48px] flex items-center justify-center">
                Broiler Starter Feed
              </h2>

              <ul className="space-y-3 text-sm text-gray-700">
                <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Protein: 22–23%
                </li>
                <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Rapid early growth and better FCR
                </li>
                {/* <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Suitable for dairy cattle & buffaloes
                </li> */}
              </ul>

              {/* <div className="space-y-2 text-[16px] mt-3">
                <div className="flex items-start gap-3">
                  <span className="flex items-center justify-center w-[18px] h-[18px] bg-green-600 rounded-full">
                    <FontAwesomeIcon
                      icon={faArrowRight}
                      className="text-white text-[10px]"
                    />
                  </span>
                  <p className="text-gray-700">Protein: 22–23%</p>
                </div>

                <div className="flex items-start gap-3">
                  <span className="flex items-center justify-center w-[18px] h-[18px] bg-green-600 rounded-full">
                    <FontAwesomeIcon
                      icon={faArrowRight}
                      className="text-white text-[10px]"
                    />
                  </span>
                  <p className="text-gray-700">
                    Rapid early growth and better FCR
                  </p>
                </div>
              </div> */}
            </div>

            {/* card-2 */}
            <div className="relative w-full h-full bg-white rounded-2xl p-8 shadow-[0_15px_40px_rgba(0,0,0,0.15)] z-30 flex flex-col">
              <span className="w-[40px] h-[40px] bg-[#00a63e] rounded-full block text-white text-center mx-auto leading-[40px] mb-4">
                <FontAwesomeIcon icon={faShieldVirus} />
              </span>

              <h2 className="text-lg font-bold text-gray-900 mb-2 text-center min-h-[48px] flex items-center justify-center">
                Broiler Grower Feed
              </h2>

              <ul className="space-y-3 text-sm text-gray-700">
                <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Protein: 20–21%
                </li>
                <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Balanced growth and muscle development
                </li>
                {/* <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Suitable for dairy cattle & buffaloes
                </li> */}
              </ul>

              {/* <div className="space-y-2 text-[18px] mt-3">
                <div className="flex items-start gap-3">
                  <span className="flex items-center justify-center w-[18px] h-[18px] bg-green-600 rounded-full">
                    <FontAwesomeIcon
                      icon={faArrowRight}
                      className="text-white text-[10px]"
                    />
                  </span>
                  <p className="text-gray-700">
                    <strong>Protein:</strong> 20–21%
                  </p>
                </div>

                <div className="flex items-start gap-3">
                  <span className="flex items-center justify-center w-[18px] h-[18px] bg-green-600 rounded-full">
                    <FontAwesomeIcon
                      icon={faArrowRight}
                      className="text-white text-[10px]"
                    />
                  </span>
                  <p className="text-gray-700">
                    Balanced growth and muscle development
                  </p>
                </div>
              </div> */}
            </div>

            {/* card-3 */}
            <div className="relative w-full h-full bg-white rounded-2xl p-8 shadow-[0_15px_40px_rgba(0,0,0,0.15)] z-30 flex flex-col">
              <span className="w-[40px] h-[40px] bg-[#00a63e] rounded-full block text-white text-center mx-auto leading-[40px] mb-4">
                <FontAwesomeIcon icon={faEgg} />
              </span>

              <h2 className="text-lg font-bold text-gray-900 mb-2 text-center min-h-[48px] flex items-center justify-center">
                Broiler Finisher Feed
              </h2>

              <ul className="space-y-3 text-sm text-gray-700">
                <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Protein: 18–19%
                </li>
                <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Efficient weight gain and market readiness
                </li>
                {/* <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Suitable for dairy cattle & buffaloes
                </li> */}
              </ul>

              {/* <div className="space-y-2 text-[18px] mt-3">
                <div className="flex items-start gap-3">
                  <span className="flex items-center justify-center w-[18px] h-[18px] bg-green-600 rounded-full">
                    <FontAwesomeIcon
                      icon={faArrowRight}
                      className="text-white text-[10px]"
                    />
                  </span>
                  <p className="text-gray-700">
                    <strong>Protein:</strong> 18–19%
                  </p>
                </div>

                <div className="flex items-start gap-3">
                  <span className="flex items-center justify-center w-[18px] h-[18px] bg-green-600 rounded-full">
                    <FontAwesomeIcon
                      icon={faArrowRight}
                      className="text-white text-[10px]"
                    />
                  </span>
                  <p className="text-gray-700">
                    Efficient weight gain and market readiness
                  </p>
                </div>
              </div> */}
            </div>

            {/* card-4 */}
            <div className="relative w-full h-full bg-white rounded-2xl p-8 shadow-[0_15px_40px_rgba(0,0,0,0.15)] z-30 flex flex-col">
              <span className="w-[40px] h-[40px] bg-[#00a63e] rounded-full block text-white text-center mx-auto leading-[40px] mb-4">
                <FontAwesomeIcon icon={faDumbbell} />
              </span>

              <h2 className="text-lg font-bold text-gray-900 mb-2 text-center min-h-[48px] flex items-center justify-center">
                Layer Feed
              </h2>

              <ul className="space-y-3 text-sm text-gray-700">
                <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Protein: 16–18%
                </li>
                <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Calcium: 3.5–4%
                </li>
                <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Supports high egg production and strong shells
                </li>
                {/* <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Suitable for dairy cattle & buffaloes
                </li> */}
              </ul>

              {/* <div className="space-y-2 text-[18px] mt-3">
                <div className="flex items-start gap-3">
                  <span className="flex items-center justify-center w-[18px] h-[18px] bg-green-600 rounded-full">
                    <FontAwesomeIcon
                      icon={faArrowRight}
                      className="text-white text-[10px]"
                    />
                  </span>
                  <p className="text-gray-700">
                    <strong>Protein:</strong> 16–18%
                  </p>
                </div>

                <div className="flex items-start gap-3">
                  <span className="flex items-center justify-center w-[18px] h-[18px] bg-green-600 rounded-full">
                    <FontAwesomeIcon
                      icon={faArrowRight}
                      className="text-white text-[10px]"
                    />
                  </span>
                  <p className="text-gray-700">
                    <strong>Calcium:</strong> 3.5–4%
                  </p>
                </div>

                <div className="flex items-start gap-3">
                  <span className="flex items-center justify-center w-[18px] h-[18px] bg-green-600 rounded-full">
                    <FontAwesomeIcon
                      icon={faArrowRight}
                      className="text-white text-[10px]"
                    />
                  </span>
                  <p className="text-gray-700">
                    Supports high egg production and strong shells
                  </p>
                </div>
              </div> */}
            </div>
          </div>

          {/* Feeding Schedule */}
          <div className="mt-14 max-w-7xl mx-auto px-4 ">
            <h3 className="text-2xl font-semibold text-gray-800 flex items-center gap-3 mb-6 text-center justify-center">
              {/* <FontAwesomeIcon icon={faClipboardList} /> */}
              Poultry Feeding Schedule
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-white rounded-2xl p-4 md:p-8 shadow-sm">
              {/* Broiler Feeding Program */}
              <div>
                <div className="mb-8">
                  <h4 className="text-lg font-semibold text-gray-800 mb-3 md:text-left text-center">
                    Broiler Feeding Program
                  </h4>

                  <div className="overflow-x-auto  border border-green-300 rounded-xl ">
                    <table className="w-full border-collapse">
                      <thead className="bg-green-100 text-gray-800">
                        <tr>
                          <th className="px-2 md:px-4 py-3 text-center text-sm md:text-base font-semibold border border-green-200">
                            Age (Days)
                          </th>
                          <th className="px-2 md:px-4 py-3 text-center text-sm md:text-base font-semibold border border-green-200">
                            Feed Type
                          </th>
                          <th className="px-2 md:px-4 py-3 text-center text-sm md:text-base font-semibold border border-green-200">
                            Feed Intake / Bird
                          </th>
                        </tr>
                      </thead>

                      <tbody className="text-gray-700">
                        <tr className="hover:bg-green-50 transition">
                          <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                            0–10 Days
                          </td>
                          <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                            Starter
                          </td>
                          <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                            250 g
                          </td>
                        </tr>

                        <tr className="hover:bg-green-50 transition">
                          <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                            11–24 Days
                          </td>
                          <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                            Grower
                          </td>
                          <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                            850 g
                          </td>
                        </tr>

                        <tr className="hover:bg-green-50 transition">
                          <td className="px-2 md:px-4 py-3 text-center border border-green-200 font-medium whitespace-nowrap">
                            25–42 Days
                          </td>
                          <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                            Finisher
                          </td>
                          <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                            1,800 g
                          </td>
                        </tr>
                      </tbody>
                    </table>
                  </div>

                  <h4 className="text-lg font-semibold text-gray-700 mt-3 md:text-left text-center">
                    Total Feed per Broiler: ~2.9 kg <br />
                    Target FCR: 1.6–1.8
                  </h4>
                </div>
              </div>

              {/* Layer Feeding Program */}
              <div>
                <h4 className="text-lg font-semibold text-gray-800 mb-3 text-center md:text-left">
                  Layer Feeding Program
                </h4>

                <div className="overflow-x-auto  border border-green-300 rounded-xl">
                  <table className="w-full border-collapse">
                    <thead className="bg-green-100 text-gray-800">
                      <tr>
                        <th className="px-4 py-3 text-center text-sm md:text-base font-semibold border border-green-200">
                          Age (Weeks)
                        </th>
                        <th className="px-4 py-3 text-center text-sm md:text-base font-semibold border border-green-200">
                          Feed Type
                        </th>
                        <th className="px-4 py-3 text-center text-sm md:text-base font-semibold border border-green-200">
                          Feed Intake / Bird
                        </th>
                      </tr>
                    </thead>

                    <tbody className="text-gray-700">
                      <tr className="hover:bg-green-50 transition">
                        <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          0–8 Weeks
                        </td>
                        <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          Chick Starter
                        </td>
                        <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          40–60 g
                        </td>
                      </tr>

                      <tr className="hover:bg-green-50 transition">
                        <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          9–18 Weeks
                        </td>
                        <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          Grower
                        </td>
                        <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          70–90 g
                        </td>
                      </tr>

                      <tr className="hover:bg-green-50 transition">
                        <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          19+ Weeks
                        </td>
                        <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          Layer Feed
                        </td>
                        <td className="px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          110–120 g
                        </td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              </div>
            </div>
          </div>
        </section>

        {/* ================= PIG ================= */}

        <section className="py-4  md:py-10">
          <div className="text-center mb-6">
            <h2 className="text-3xl md:text-5xl font-semibold text-gray-800 text-center ">
              Pig <span className="text-[#ffa800]">Feed</span>
            </h2>

            <p className="text-gray-600 mt-3 max-w-2xl mx-auto text-[16px] md:text-[18px]">
              Proper feeding at the right age and quantity ensures better
              growth, lower feed cost, and higher productivity.
            </p>
          </div>

          <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 items-stretch gap-4 md:gap-6">
            {/* card-1 */}
            <div className="relative w-full h-full bg-white rounded-2xl p-8 shadow-[0_15px_40px_rgba(0,0,0,0.15)] z-30 flex flex-col">
              <span className="w-[40px] h-[40px] bg-[#00a63e] rounded-full block text-white text-center mx-auto leading-[40px] mb-4">
                <FontAwesomeIcon icon={faPiggyBank} />
              </span>

              <h2 className="text-lg font-bold text-gray-900 mb-2 text-center min-h-[48px] flex items-center justify-center">
                Pig Starter Feed
              </h2>

              <ul className="space-y-3 text-sm text-gray-700">
                <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Protein: 20-22%
                </li>
                <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Improves early growth and gut health
                </li>
              </ul>
            </div>

            {/* card-2 */}
            <div className="relative w-full h-full bg-white rounded-2xl p-8 shadow-[0_15px_40px_rgba(0,0,0,0.15)] z-30 flex flex-col">
              <span className="w-[40px] h-[40px] bg-[#00a63e] rounded-full block text-white text-center mx-auto leading-[40px] mb-4">
                <FontAwesomeIcon icon={faDrumstickBite} />
              </span>

              <h2 className="text-lg font-bold text-gray-900 mb-2 text-center min-h-[48px] flex items-center justify-center">
                Pig Grower Feed
              </h2>

              <ul className="space-y-3 text-sm text-gray-700">
                <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Protein: 18-20%
                </li>
                <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Faster weight gain with better feed conversion
                </li>
              </ul>
            </div>

            {/* card-3 */}
            <div className="relative w-full h-full bg-white rounded-2xl p-8 shadow-[0_15px_40px_rgba(0,0,0,0.15)] z-30 flex flex-col">
              <span className="w-[40px] h-[40px] bg-[#00a63e] rounded-full block text-white text-center mx-auto leading-[40px] mb-4">
                <FontAwesomeIcon icon={faShieldVirus} />
              </span>

              <h2 className="text-lg font-bold text-gray-900 mb-2 text-center min-h-[48px] flex items-center justify-center">
                Pig Finisher Feed
              </h2>

              <ul className="space-y-3 text-sm text-gray-700">
                <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Protein: 16-18%
                </li>
                <li className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                    <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                  </span>
                  Cost-effective finishing and improved carcass quality
                </li>
              </ul>
            </div>
          </div>

          {/* Feeding Schedule */}

          <div className="mt-14 max-w-7xl mx-auto px-4">
            <h3 className="text-2xl font-semibold text-gray-800 flex items-center gap-3 mb-6 text-center justify-center">
              Pig Feeding Schedule
            </h3>

            <div className="bg-white rounded-2xl p-4 md:p-8 shadow-sm ">
              <div className="mb-8">
                <h4 className="text-lg font-semibold text-gray-800 mb-3 text-center md:text-left">
                  Pig Growth Stage
                </h4>

                {/* table wrapper */}
                <div className="overflow-x-auto border border-green-300 rounded-xl ">
                  <table className="w-full border-collapse">
                    <thead className="bg-green-100 text-gray-800">
                      <tr>
                        <th className="px-2 md:px-4 py-3 text-center text-sm md:text-base font-semibold border border-green-200">
                          Age (Weeks)
                        </th>
                        <th className="px-2 md:px-4 py-3 text-center text-sm md:text-base font-semibold border border-green-200">
                          Feed Type
                        </th>
                        <th className="px-2 md:px-4 py-3 text-center text-sm md:text-base font-semibold border border-green-200">
                          Body Weight
                        </th>
                        <th className="px-2 md:px-4 py-3 text-center text-sm md:text-base font-semibold border border-green-200">
                          Feed / Day
                        </th>
                      </tr>
                    </thead>

                    <tbody className="text-gray-700">
                      <tr className="hover:bg-green-50 transition">
                        <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          6–8 Weeks
                        </td>
                        <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          Starter
                        </td>
                        <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          8–25 kg
                        </td>
                        <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          1.0–1.5 kg
                        </td>
                      </tr>

                      <tr className="hover:bg-green-50 transition">
                        <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          8–16 Weeks
                        </td>
                        <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          Grower
                        </td>
                        <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          25–60 kg
                        </td>
                        <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          2.0–2.5 kg
                        </td>
                      </tr>

                      <tr className="hover:bg-green-50 transition">
                        <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          16–24 Weeks
                        </td>
                        <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          Finisher
                        </td>
                        <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          60–100 kg
                        </td>
                        <td className="px-2 md:px-4 py-3 text-center border border-green-200 whitespace-nowrap">
                          2.5–3.5 kg
                        </td>
                      </tr>
                    </tbody>
                  </table>
                </div>
                {/* end table */}
              </div>
            </div>
          </div>
        </section>

        {/* ================= FISH ================= */}
        <section className="py-4 md:py-10">
          <div className="text-center  mb-6">
            {/* <h2 className="text-3xl md:text-5xl font-semibold text-green-800 flex items-center justify-center gap-3">
                Feed
              </h2> */}

            <h2 className="text-3xl md:text-5xl font-semibold text-gray-800 text-center ">
              Fish <span className="text-[#ffa800]">Feed</span>
            </h2>
          </div>
          <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-2 items-center gap-4 md:gap-12">
            <div className="  order-2 md:order-1">
              <div className="space-y-3">
                <div
                  className="relative w-full h-auto bg-white rounded-2xl p-8 space-y-5
                     shadow-[0_15px_40px_rgba(0,0,0,0.15)] z-30"
                >
                  <h3 className="text-[22px] md:text-[24px] font-bold text-gray-800">
                    Floating Fish Feed
                  </h3>

                  <ul className="space-y-3 text-sm text-gray-700">
                    <li className="flex items-center gap-3">
                      <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                        <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                      </span>
                      Protein: 28–32%
                    </li>
                    <li className="flex items-center gap-3">
                      <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                        <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                      </span>
                      Better feed intake monitoring
                    </li>
                    <li className="flex items-center gap-3">
                      <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                        <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                      </span>
                      Reduces water pollution
                    </li>
                  </ul>
                </div>
                <div
                  className="relative w-full h-auto bg-white rounded-2xl p-8 space-y-5
                     shadow-[0_15px_40px_rgba(0,0,0,0.15)] z-30"
                >
                  <h3 className="text-[22px] md:text-[24px] font-bold text-gray-800">
                    Sinking Fish Feed
                  </h3>

                  <ul className="space-y-3 text-sm text-gray-700">
                    <li className="flex items-center gap-3">
                      <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                        <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                      </span>
                      Protein: 24–28%
                    </li>

                    <li className="flex items-center gap-3">
                      <span className="flex items-center justify-center w-[10px] h-[10px] bg-green-600 rounded-full">
                        <i className="fa-solid fa-arrow-right text-white text-[10px]"></i>
                      </span>
                      Sinking Fish Feed
                    </li>
                  </ul>
                </div>
              </div>

              {/* <div className="space-y-3">
                <div
                  className="relative w-full h-auto bg-white rounded-2xl p-8 space-y-5
                     shadow-[0_15px_40px_rgba(0,0,0,0.15)] z-30"
                >
                  <h3 className="text-[22px] md:text-[24px] font-bold text-gray-800">
                    Floating Fish Feed
                  </h3>

                  <ul className="space-y-2 text-[16px] text-gray-700">
                    <li className="flex items-center gap-3">
                      <span className="flex items-center justify-center w-[18px] h-[18px] bg-green-600 rounded-full">
                        <FontAwesomeIcon
                          icon={faArrowRight}
                          className="text-white text-[10px]"
                        />
                      </span>
                      <strong>Protein:</strong> 28–32%
                    </li>
                    <li className="flex gap-2">
                      <span className="flex items-center justify-center w-[18px] h-[18px] bg-green-600 rounded-full">
                        <FontAwesomeIcon
                          icon={faArrowRight}
                          className="text-white text-[10px]"
                        />
                      </span>
                      Better feed intake monitoring
                    </li>
                    <li className="flex gap-2">
                      <span className="flex items-center justify-center w-[18px] h-[18px] bg-green-600 rounded-full">
                        <FontAwesomeIcon
                          icon={faArrowRight}
                          className="text-white text-[10px]"
                        />
                      </span>
                      Reduces water pollution
                    </li>
                  </ul>
                </div>
                <div
                  className="relative w-full h-auto bg-white rounded-2xl p-8 space-y-5
                     shadow-[0_15px_40px_rgba(0,0,0,0.15)] z-30"
                >
                  <h3 className="text-[22px] md:text-[24px] font-bold text-gray-800">
                    Sinking Fish Feed
                  </h3>

                  <ul className="space-y-2 text-[16px] text-gray-700">
                    <li className="flex items-center gap-3">
                      <span className="flex items-center justify-center w-[18px] h-[18px] bg-green-600 rounded-full">
                        <FontAwesomeIcon
                          icon={faArrowRight}
                          className="text-white text-[10px]"
                        />
                      </span>
                      <strong>Protein:</strong> 24–28%
                    </li>
                    <li className="flex gap-2">
                      <span className="flex items-center justify-center w-[18px] h-[18px] bg-green-600 rounded-full">
                        <FontAwesomeIcon
                          icon={faArrowRight}
                          className="text-white text-[10px]"
                        />
                      </span>
                      Suitable for bottom-feeding species
                    </li>
                  </ul>
                </div>
              </div> */}
            </div>
            <div className=" order-1 md:order-2">
              <img
                src={animal4}
                className=" w-full  h-[280px]  md:h-[350px] object-cover  rounded-2xl "
                alt=""
              />
            </div>
          </div>
        </section>
      </main>
      <Footer></Footer>
    </>
  );
}

export default FeedType;