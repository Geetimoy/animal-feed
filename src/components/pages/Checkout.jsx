...
export default function Checkout() {
  ...
  // Total Amount (Subtotal + Delivery + Tax)
  ...
  const fetchAddresses = async () => {
    try {
      ...
      if (addressList.length > 0 && !selected) {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      // Convert shipping_address to string
      const addressParts = [
      ...
      if (response.data.success || response.data.data) {
        ...
        localStorage.setItem('lastOrderDetails', JSON.stringify({
          orderId: response.data.data?.order_id || response.data.order_id || 'N/A',
          status: 'Processing',
      ...
      <main className="pt-16 bg-[#f7f8fa] min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-12 mt-16">
          ...
          <div className="md:col-span-2 space-y-6">
                ...
                <div className="">
                    ...
                    <div>
                      ...
                      {selected && current ? (
                        <>
                          ...
                          {current.landmark && (
                            <p className="mt-1 text-gray-600">Near: {current.landmark}</p>
                          )}
                          <p className="mt-1 text-gray-600">
                            {current.city || ''} {current.pincode || ''}
                          </p>
                        </>
                      ) : (
                  ...
                  <div className={`transition-all duration-300 overflow-hidden ${panelopen ? "max-h-auto p-4" : "max-h-0"}`}>
                    {addresses.map((item) => (
                      <label
                        key={item.id}
                        className={`flex gap-3 p-3 border rounded-lg mb-3 cursor-pointer
                          ${selected === item.id ? "border-green-500 bg-blue-50" : "border-gray-200"}`}
                      >
                        <input
                          type="radio"
                    ...
                    {!showAddressForm ? (
                      <button
                        ...
                        className="w-full mt-3 py-3 border-2 border-dashed border-green-500 rounded-lg text-green-600 font-medium hover:bg-green-50 transition cursor-pointer flex items-center justify-center gap-2"
                      >
                      ...
                      <div className="mt-4 p-4 border border-green-500 rounded-lg bg-green-50">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          ...
                          <textarea
                            ...
                            className="md:col-span-2 border border-gray-300 rounded-md px-4 py-3 text-md focus:ring-2 focus:ring-green-600 focus:outline-none resize-none"
                          ></textarea>
                          ...
                          <div className="md:col-span-2 flex gap-3">
                            <button
                              ...
                              className="px-6 py-3 bg-gradient-to-r from-[#00a34a] to-[#009a62] text-white rounded-[12px] hover:opacity-90 transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
                            >
                              {savingAddress ? (
                                <>
                            ...
                            <button
                              onClick={() => setShowAddressForm(false)}
                              className="px-6 py-3 bg-gray-200 text-gray-700 rounded-[12px] hover:bg-gray-300 transition cursor-pointer"
                            >
                              Cancel
                            </button>
                          </div>
            ...
            <div className="bg-white rounded-[12px] shadow-md py-4 px-4">
              <div className="flex flex-col gap-2 p-4">
                ...
                <textarea
                  ...
                  className="border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-green-500 w-full"
                  rows="3"
                />
              </div>
            </div>
            ...
            <div className="md:flex items-start justify-between hidden md:block">
              <button
                ...
                className="bg-yellow-500 text-white py-3 rounded-xl font-medium cursor-pointer hover:bg-yellow-400 text-[16px] px-8 hover:opacity-90 transition"
              >
                Continue Shopping
              </button>
              <button
                onClick={() => navigate("/cart")}
                className="bg-yellow-500 text-white py-3 rounded-xl font-medium cursor-pointer hover:bg-yellow-400 text-[16px] px-8 hover:opacity-90 transition ml-4"
              >
                Back to Cart
              </button>
            </div>
          ...
          <div className="bg-white p-6 rounded-[12px] shadow-md h-fit">
            ...
            <button
              ...
              className={`w-full py-3 rounded-xl font-medium text-[16px] mt-6 flex items-center justify-center gap-2 transition
                ${loading || cartItems.length === 0 || !selected
          ...
          <div className="flex items-center flex-col-reverse gap-4 md:hidden">
            <button
              onClick={() => navigate("/products")}
              className="w-full bg-yellow-500 text-white py-3 rounded-xl font-medium cursor-pointer hover:bg-yellow-400 text-[16px] px-8 hover:opacity-90 transition"
            >
              Continue Shopping
            </button>
            <button
