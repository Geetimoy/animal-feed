...
export default function NewsTags() {
  ...
  const { seo } = usePageSEO(
    tagName ? `news/tag/${tagName}` : null
  );

  const pageSlug = `news/tag/${tagName}`;
  const {
  bannerItem,
  ...
  const fetchTagsNews = async () => {
    try {
      setLoading(true);

      const res = await axios.get(
        `${API_URL}/news/tag/${tagName}`
      );
    ...
    <>
      ...
      <main className="pt-16 overflow-hidden">
        ...
        <section className="max-w-7xl mx-auto px-4 py-12">
          ...
          <h2 className="text-3xl md:text-5xl font-semibold text-gray-800 text-center capitalize">
            {tag} <span className="text-[#ffa800]">Articles</span>
          </h2>
          <p className="text-gray-500 mb-10 text-center mt-4">
